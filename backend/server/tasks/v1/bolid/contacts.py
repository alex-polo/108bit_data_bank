...
async def contacts_parse(url: str, headers: dict) -> ReturnedValue:
    try:
        ...
        if response.is_success:
            logger.info('Download Bolid complete')
            download_data: LoaderValue = response.data

            soup = BeautifulSoup(download_data.content, 'html.parser')
            addr = soup.find('div', class_='contacts_right')
            item = addr.find('div', {'itemprop': 'address'})
            ...
            error_data.message = f'Parsing contacts for Bolid is uncompilable, {error_data.message}'
            return ReturnedValue(
                is_success=False,
                data=error_data
            )
        ...
        return ReturnedValue(
            is_success=False,
            data=ErrorValue(
                message=f'Parsing contacts for Bolid is uncompilable, error: {str(error)}',

...
async def contacts_parse(url: str, headers: dict) -> ReturnedValue:
    try:
        ...
        if response.is_success:
            ...
            error_data.message = f'Parsing contacts for Macroscop is uncompilable, {error_data.message}'
            return ReturnedValue(
                is_success=False,
                data=error_data
            )
        ...
        return ReturnedValue(
            is_success=False,
            data=ErrorValue(
                message=f'Parsing contacts for Macroscop is uncompilable, error: {str(error)}',
